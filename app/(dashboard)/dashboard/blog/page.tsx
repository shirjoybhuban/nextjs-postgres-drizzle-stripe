...
export default function BlogPostPage() {
  ...
  const blogColumns: ColumnDef<Blog>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <div className="text-center text-xs font-semibold text-green-200 bg-slate-800 px-2 py-1 rounded">
            {row.getValue("status")}
          </div>
        )
      },
    ...
    <section className="flex-1 p-4 lg:p-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          {blogs?.totalCount > 0 && (
            <p className="text-xs lg:text-sm font-medium text-gray-900 mb-6">
              {blogs?.totalCount} entries found
            </p>
          )}
        </div>
